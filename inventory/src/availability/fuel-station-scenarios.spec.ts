...
describe('Fuel Station Scenarios (Pool Availability)', () => {
    ...
    describe('Reserving fuel', () => {
        ...
        it('multiple fleet companies can reserve from same tank', () => {
            ...
            expect(fleetAResult.isSuccess()).toBe(true);
            expect(fleetBResult.isSuccess()).toBe(true);
            expect(taxiResult.isSuccess()).toBe(true);
        });
        ...
        it('same company can make multiple reservations', () => {
            ...
            expect(firstVehicle.isSuccess()).toBe(true);
            expect(secondVehicle.isSuccess()).toBe(true);
    ...
    describe('Cancelling reservations', () => {
        ...
        it('cancelled fuel becomes available for others', () => {
            ...
            expect(facade.lockPool(PETROL_95_TANK, PoolLockRequest.indefinite(PETROL_95_TANK, Quantity.of(1500, LITERS), FLEET_COMPANY_B)).isFailure()).toBe(true);
    ...
    describe('Tank capacity edge cases', () => {
        ...
        it('multiple small reservations can fill the tank', () => {
            ...
            expect(lastReservation.isSuccess()).toBe(true);
            expect(facade.isAvailable(tank.id())).toBe(false);
            expect(facade.lockPool(PETROL_95_TANK, PoolLockRequest.indefinite(PETROL_95_TANK, Quantity.of(1, LITERS), FLEET_COMPANY_A)).isFailure()).toBe(true);
        });
    });
});
